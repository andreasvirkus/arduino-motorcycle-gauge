...
byte ST7565::setFont (const byte* font) {
...
byte ST7565::writestring(byte x, byte y, const char *c) {
    while (*c) {
        byte w = writechar(x, y, *c++);
        x += w;
        if (x + w >= LCDWIDTH) {
...
byte ST7565::writechar(byte x, byte y, char c) {
